...
from tvm.topi import math
from tvm.meta_schedule.testing import te_workload

from tvm.topi.x86.batch_matmul import batch_matmul as decl_batch_matmul, schedule_batch_matmul
...
from tvm.topi.x86.conv2d import conv2d_nchw as decl_conv2d_nchw, schedule_conv2d_nchw
...
from tvm.topi.x86.conv3d import conv3d_ncdhw as decl_conv3d_ncdhw, schedule_conv3d_ncdhw
...
def conv3d_ncdhw(N, D, H, W, CI, CO, kernel_size, stride=1, padding=0, dilation=1, groups=1):
    ...
    output = decl_conv3d_ncdhw(inputs, weight, (stride, stride, stride),
            (padding, padding, padding), dilation, groups, 'float32')
    s = schedule_conv3d_ncdhw([output])

    return s, [inputs, weight, output]

from tvm.topi.x86.depthwise_conv2d import depthwise_conv2d_nchw as decl_depthwise_conv2d_nchw, schedule_depthwise_conv2d_nchw
...
from tvm.topi.x86.conv2d_transpose import conv2d_transpose_nchw as decl_conv2d_transpose_nchw, schedule_conv2d_transpose_nchw
...
def conv2d_transpose_nchw(N, H, W, CI, CO, kernel_size, stride=1, padding=0):
    ...
    output = decl_conv2d_transpose_nchw(inputs, weight, strides=(stride, stride), padding=(padding, padding), out_dtype='float32', output_padding=(0, 0))
...
from tvm.topi.x86.reduction import schedule_reduce as x86_schedule_reduce
...
from tvm.topi.cuda.conv2d import conv2d_nchw as decl_conv2d_nchw_cuda, schedule_conv2d_nchw as schedule_conv2d_nchw_cuda
...
from tvm.topi.cuda.batch_matmul import batch_matmul as decl_batch_matmul_cuda, schedule_batch_matmul as schedule_batch_matmul_cuda
...
from tvm.topi.cuda.injective import schedule_injective_from_existing as schedule_injective_from_existing_cuda
...
task_func_dict = {
    ...
    'C2d-BN-RELU': (conv2d_nchw_bn_relu, "conv2d_nchw_bn_relu"),
    'TBG': (transpose_batch_matmul, "transpose_batch_matmul"),
...
subgraph_eval_wkls = ['C2d-BN-RELU', 'TBG']
...
if __name__ == "__main__":
    ...
    for wkl_meta_name in wkl_meta_names:
        ...
        for batch_size in batch_sizes:
            for shape in te_workload.CONFIGS[wkl_meta_name][1][:1]:
                if shape[0] == 1:
                    shape = list(shape)
                    shape[0] = batch_size
                    shape = tuple(shape)
                ...
                tuner.tune(n_trial=args.n_trials,
                           ...
                           callbacks=[
                               autotvm.callback.progress_bar(args.n_trials, prefix=wkl_meta_name),

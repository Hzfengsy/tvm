...
TUNING = True
log_file = "conv2d_tensorcore_packed.log"
...
def conv2d_with_tensorcore(batch_size, height, width, in_channels, out_channels,
    ...
    output_shape = (batch_size // wmma_m,
                    height,
                    width,
                    out_channels // wmma_n,
                    wmma_m,
    ...
    tx, xo = s[WS].split(o, nparts=cfg["block_row_warps"].val)
    ty, yo = s[WS].split(xo, nparts=cfg["block_col_warps"].val)
    t = s[WS].fuse(ii, oo)
    to, ti = s[WS].split(t, factor=warp_size)
    s[WS].bind(tx, thread_y)
    s[WS].bind(ty, thread_z)
    s[WS].bind(ti, thread_x)

    s[AF].tensorize(AF.op.axis[-2], intrin_wmma_load_matrix_A('wmma.matrix_a', wmma_m, wmma_k, wmma_n))
    s[WF].tensorize(WF.op.axis[-2], intrin_wmma_load_matrix_W('wmma.matrix_b', wmma_m, wmma_k, wmma_n))
    s[Conv].tensorize(nnc, intrin_wmma_store_matrix(wmma_m, wmma_k, wmma_n))
    s[ConvF].tensorize(nnf, intrin_wmma_gemm(wmma_m, wmma_k, wmma_n))
